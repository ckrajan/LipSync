                ...
                if not results.detections:
                    ...
                    for detection in results.detections: # iterate over each detection and draw on image
                        ...
                        if(keypoints1 is not None):
                            ...
                            # 93, 323, 140, 378
                            # mouth_points = {
                            #     "xmin" : int(ls_single_face1[93].x * width),
                            #     "ymin" : int(ls_single_face1[93].y * height),
                            #     "xmax" : int(ls_single_face1[378].x * width),
                            #     "ymax" : int(ls_single_face1[378].y * height)
                            # }
                            ...
                            ymin1 = int((mouth_points["ymin"] + mouth_points["ymax"]) / 2) + 50

                            mask = np.zeros(image.shape, dtype=np.uint8) 
                            cv2.rectangle(mask, (mouth_points["xmin"], ymin1), (mouth_points["xmax"], mouth_points["ymax"]), (255, 255, 255), -1)
